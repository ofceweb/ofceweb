...
| `DEPLOY_PAT` | Lue **en priorité** par [`site2branch()`](https://ofceweb.github.io/ofceweb/reference/site2branch.md) / [`trigger_action()`](https://ofceweb.github.io/ofceweb/reference/trigger_action.md) pour pousser sur la branche de déploiement et dispatcher le workflow `ftp_deploy.yml`. |
...
retombent sur le keystore du système (Keychain sur macOS, Credential
Manager sur Windows) si `DEPLOY_PAT` n’est pas défini :
