...
#'
#' Équivalent PB de [push_wp_redirect()]. Génère un `index.html` de redirection
...
#' @seealso [deploy_pb()], [publish_pb()], [pb_version_up()]
...
#' @keywords internal
push_pb_redirect <- function(path = ".", progress = TRUE, trigger = TRUE) {
  ...
  if (is.null(yml$pb)) {
    cli::cli_alert_info("PB brouillon — pas de redirection stable à déployer.")
...
#' Déploie la page de redirection stable pour le PB en staging
#'
#' Équivalent PB de [push_wp_staging_redirect()]. Génère un `index.html` de
#' redirection pointant vers la version courante d'un PB en staging FTP (lue
#' depuis `website.site-url` de `_quarto.yml`, de la forme
...
#' [setup_pb()]).
#'
#' Ne fait rien si `website.site-url` n'a pas la forme attendue (PB publié,
...
#'
#' @returns Invisible `NULL`. Appelée pour ses effets de bord.
#' @seealso [deploy_pb()], [push_pb_redirect()], [pb_version_up()]
#' @importFrom fs path_expand path_abs path_norm path file_exists dir_create dir_delete
...
#' @importFrom glue glue
#' @keywords internal
push_pb_staging_redirect <- function(path = ".", progress = TRUE, trigger = TRUE) {
  ...
  if (!isTRUE(yml$ofce_pb))
    cli::cli_abort(
      "{.fn push_pb_staging_redirect} ne fonctionne que sur un dépôt PB \\
       ({.code ofce_pb: true} absent du {.file _quarto.yml}).")
  ...
  # cette forme (PB publié -> "https://www.ofce.fr/", brouillon GitHub Pages
