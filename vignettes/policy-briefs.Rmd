---
title: "Policy briefs (PB)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Policy briefs (PB)}
...
travail (WP)](working-papers.html) : mêmes étapes (initialisation,
vérification, rendu, déploiement, incrémentation de version), fonctions
`pb_*` équivalentes aux fonctions `wp_*`, mêmes gabarits Quarto
(`_quarto.yml`, `index.qmd`, `annexes.qmd`, `news.qmd`) et les mêmes
...
|---|---|---|
| Fonctions | `setup_wp()`, `check_wp()`, `render_wp()`, `deploy_wp()`, `publish_wp()`, `wp_version_up()`, `wp_registry_request()` | `setup_pb()`, `check_pb()`, `render_pb()`, `deploy_pb()`, `publish_pb()`, `pb_version_up()`, `pb_registry_request()` |
...
`setup_pb()` refuse d'être appliqué à un dépôt marqué `ofce_wp: true` ou
`ofce_prev: true` (et réciproquement pour `setup_wp()`) : un dépôt a un
type unique.

### Cycle de vie

+-------------+---------------+-------------+----------------------------------------+
| État        | `pb` dans     | Hébergement | URL                                    |
|             | `_quarto.yml` |             |                                        |
+=============+===============+=============+==========================================+
| Brouillon   | `null`        | GitHub      | `ofce.github.io/{repo}/` ou            |
|             |               | Pages ou    | `staging.ofce.fr/{repo}/{version}/`    |
|             |               | OFCE        |                                         |
|             |               | staging     |                                         |
+-------------+---------------+-------------+----------------------------------------+
| Publié      | `N` (entier)  | Serveur     | `www.ofce.fr/pb/{N}/{version}/`        |
|             |               | OFCE FTP    |                                         |
+-------------+---------------+-------------+----------------------------------------+
...
l'organisation OFCE. Convention de nommage recommandée : `pb-{initiale de
l'auteur.e}-{nom court du projet}`, tout en minuscule (ex. `pb-gaxt-relance`).
...
`setup_pb()` suit le même pipeline que `setup_wp()` : vérification de la
connexion GitHub, initialisation de la branche `gh-pages`, copie des
gabarits (`_quarto.yml`, `index.qmd`, `annexes.qmd`, `news.qmd`) et des
assets OFCE, installation/mise à jour des extensions Quarto OFCE, mise à
jour forcée des workflows GitHub Actions.

Différence notable : `pb` n'est **pas un argument** de `setup_pb()`
(contrairement à `wp`/`annee` pour `setup_wp()`) : il est toujours lu
depuis `_quarto.yml`, ou écrasé par une entrée confirmée du registre
central. Un dépôt sans `_quarto.yml` et sans entrée de registre reste un
brouillon (`pb` absent) — pour obtenir un numéro, voir la [section
5](#passer-de-brouillon-a-publie) ci-dessous. Le champ `annee` n'est
jamais utilisé pour les PB.

Après `setup_pb()`, éditez `_quarto.yml` et `index.qmd` pour les
métadonnées définitives (auteurs, résumé, date) et le contenu avant de
commiter.
...
`check_pb()` vérifie les mêmes catégories de champs que `check_wp()`
...
Elle retourne un data frame de diagnostics et est appelée automatiquement
par `render_pb()`. `complete_pb_yaml()` (équivalent de `complete_wp()`)
...
Pipeline identique à `render_wp()` (vidage de `_site/`, rendu HTML + PDF
via `quarto::quarto_render()`, reconstruction du sitemap, écriture de
`manifest.json`, synchronisation de `FTP_SERVER_DIR`, réécriture des
liens absolus en relatifs, prévisualisation locale via
`servr::httw()`), à une différence près : **`render_pb()` ne consulte
plus le registre central** — cette consultation a lieu en amont, dans
`setup_pb()` (et à nouveau dans `publish_pb()`). `render_pb()` suppose
que `draft`/`pb` sont déjà synchronisés dans `_quarto.yml` et se contente
de les lire, sans accès réseau.
...
écrit par `render_pb()`) et, avant publication, de `stage-target` dans
`_quarto.yml` :
...
## 5. Passer de brouillon à publié {#passer-de-brouillon-a-publie}
...
2.  Une fois la PR fusionnée, relancer `setup_pb()` pour synchroniser
    `pb`/`draft`/`site-path`/`citation.*`/`stable_url` (`citation.url`/`stable_url` :
    `https://www.ofce.fr/pb/{N}/`, `citation.issue`: `"{N}"`) et les
    variables FTP :
...
`publish()` (qui enchaîne `render()` et `deploy()`) refait cette
consultation du registre juste avant le rendu, pour rattraper un
enregistrement survenu depuis le dernier `setup_pb()` — mais ne
recalcule que `draft`/`pb`, pas `site-path`/`citation.*` : si le numéro
change à cette étape, relancer `setup_pb()` avant de publier à nouveau.
...
secret `STATICRYPT_PASSWORD` sur le dépôt (voir la vignette
*Documents de travail (Working Papers)*, section 6).
...
`pb_manifest()` (appelée automatiquement par `render_pb()`) génère
`manifest.json`, collecté par `webhome` pour construire l'index des PB de
l'OFCE :
