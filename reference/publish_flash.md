...
renders \`path\` as a standalone Quarto site, then immediately pushes
and deploys it to \`staging.ofce.fr\`. This is the typical entry point
for ad-hoc folder publishing. Call \[render_flash()\] and
\[deploy_flash()\] separately instead when you want to iterate on the
render (e.g. via \[preview_flash()\]) before deploying.
...
- slug:
  ...
  (default), \[render_flash()\] computes it and persists it in
  \`\_site/.adhoc-meta.json\`, from which \[deploy_flash()\] then reads
...
- as_job:
  ...
  in the current session. The local preview server (\[preview_flash()\])
  is not launched in either mode — call it separately if you want a
  local preview.
...
\[render_flash()\], \[deploy_flash()\], \[preview_flash()\]
