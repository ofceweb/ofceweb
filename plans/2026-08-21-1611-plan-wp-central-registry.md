...
3.  Fetch `registry.json` from `wp-registry`'s default branch (plain `curl`, no auth needed since the registry is public).
4.  Look up the entry for `{annee, wp}`:
...
### Staging = FTP (chrooted staging credentials), publish = FTP (production credentials) — revised 2026-08-25

**Revised 2026-08-25:** the 2026-08-23 decision to host staging entirely on GitHub Pages is superseded. The motivation for reversing it: GitHub Pages is single-state (each push overwrites the previous version), making it unfit for academic review cycles where multiple draft versions must coexist and be linkable independently (e.g. v1 sent to reviewer A, v2 sent to reviewer B). FTP staging, by contrast, supports versioned paths natively and is consistent with the final production infrastructure.

**Server-side confirmation (2026-08-25):** `STAGING_USERNAME` (the staging FTP credential, already used by `setup_prev`’s `ftp_deploy_staging.yml`) is chrooted to `stage/` on the FTP server. This means the staging credentials can be distributed as org-level secrets to all `ofce` repos without risk of any repo escaping to the production (`wp/`) tree — even if a repo’s `FTP_STAGING_DIR` variable were set to an arbitrary path, the chroot enforces the boundary server-side.

Two-tier FTP credential model:

| Credential pair | Path scope (server-side) | When used |
|---|---|---|
| `STAGING_USERNAME` / `STAGING_PASSWORD` | Chrooted to `stage/` | Pre-registration staged deploys |
| `WP_USER` / `WP_PASSWORD` | `wp/` (production) | Registry-confirmed numbered deploys only |

Staging model details:

- **Staging path:** `stage/wp/{repo-slug}/v{n}/`, controlled by a `FTP_STAGING_DIR` repo variable set by `setup_wp()` / `render_wp()`. Version suffix matches the WP’s `version` field (the same `v{n}` already managed by `wp_version_up()`), so multiple draft versions coexist as separate subpaths.
- **No `ofce`-org-membership gate for staging.** The chroot makes credential distribution safe regardless of caller org; the org-membership check in `ftp_deploy.yml` applies only to production publishing with `WP_USER`/`WP_PASSWORD`. A non-`ofce` repo can stage a draft indefinitely but can never proceed to numbered production publishing while outside `ofce`.
- **Encryption optional.** Staticrypt on staged output is available via the existing opt-in `STATICRYPT_PASSWORD` secret mechanism but is not required by default.
- **GitHub Pages** remains useful for the truly initial pre-request draft state (before `wp_registry_request()` is called and `annee`/`wp` are assigned), where no FTP staging path exists yet. Once a number is requested/assigned, FTP staging takes over for versioned review distribution.
- **"Published" is defined purely by registry state.** A repo is published if and only if a `type: "repo"` entry in `registry.json` matches its `source-repo`. The very next render/publish cycle after merge fetches the registry, finds the match, and switches from staging credentials + staging path to production credentials + numbered path.
- **Cleanup:** unlike GitHub Pages, FTP staging folders are not garbage-collected on re-render. A retention/cleanup policy is now needed — see open decision 14.
...
### Exact staging-flag mechanism — revised 2026-08-25

**Architecture correction (carried over from 2026-08-22):** `ftp_deploy.yml` does **not itself render anything** — it checks out already-rendered static output from the `site-deploy` branch and uploads it via FTP. Actual rendering happens earlier, either locally via `render_wp()` or in CI via `render_and_deploy.yml`'s `Rscript -e "ofceweb::render_wp(...)"` step. **A banner baked into the HTML/PDF/Typst output must therefore be decided at render time, not inside `ftp_deploy.yml`.** (Related finding: `render_and_deploy.yml` currently has no anti-collision/registry check at all; placing the check in `render_wp()` as below closes that gap.)

Revised mechanism (2026-08-25), reflecting FTP staging rather than GitHub Pages for the `stage = TRUE` path:

1.  **`render_wp()` performs the registry check itself**, before calling `quarto::quarto_render()`: fetch `registry.json`, resolve this repo's `source-repo` (reusing `gh_slug_from_remote()`), and look for a matching `type: "repo"` entry. **`{annee, wp}` are taken from the matched entry, not from local `_quarto.yml`.** Result: a boolean `stage <- TRUE`/`FALSE`:
    - `stage = TRUE` (no registry match): no `{annee, wp}` assigned; output is deployed to the FTP staging path (`stage/wp/{repo-slug}/v{n}/`) using `STAGING_USERNAME`/`STAGING_PASSWORD` (chrooted to `stage/`).
    - `stage = FALSE` (confirmed registry match): `{annee, wp}` set from the registry entry; output is deployed to the numbered FTP production path using `WP_USER`/`WP_PASSWORD`.
2.  **Pass `stage` into the render as Quarto metadata:** `quarto::quarto_render(output_format = "all", metadata = list(stage = stage))`. `ofce-quarto-extensions`'s format templates read `{{< meta stage >}}` (or a Lua filter reading `meta.stage`) to conditionally show the "Version provisoire — non publiée" banner/watermark in each of `wp-html`/`wp-pdf`/`wp-typst` — this is the concrete hook the "Template changes" section above needs.
3.  **Persist `stage`, `annee`, and `wp` in `manifest.json`** (extend `wp_manifest()`'s schema with a `"stage": true/false` field, alongside `annee`/`wp` now always sourced from the registry match rather than local config). The `stage` field in the manifest drives which credential pair and FTP path the deploy workflow uses — staging credentials + `FTP_STAGING_DIR` for `stage = TRUE`, production credentials + `FTP_SERVER_DIR` for `stage = FALSE`. This value is informational/traceable; authorization is re-checked fresh at deploy time (see step 4).
4.  **Deploy-time checks differ by path:**
    - **Staging deploy (`stage = TRUE`):** no registry check needed — the server-side chroot on `STAGING_USERNAME` is the security boundary. `ftp_deploy.yml` (or a dedicated `ftp_stage.yml`) simply uses the staging credentials and `FTP_STAGING_DIR`; it does not need to verify the registry because the chroot physically prevents writes outside `stage/`.
    - **Production deploy (`stage = FALSE`):** `ftp_deploy.yml` still performs its own fresh registry check immediately before using `WP_USER`/`WP_PASSWORD` — org-membership check, then registry match. It does not trust the manifest's `stage`/`annee`/`wp` values for authorization; if the fresh check no longer confirms a match (entry removed, repo deregistered, etc. since the last render), it aborts rather than publishing.
...
3.  ~~Keep Phase 1's live-manifest check alongside the new registry check, or retire it once the registry is authoritative?~~ **Resolved 2026-08-25: keep for now, deprecate later.** The live-manifest check (Phase 1) is retained alongside the registry check as defense-in-depth during the transition period. Once the central registry logic is fully implemented and proven stable, the live-manifest check can be deprecated and removed — that deprecation is a follow-up task, not part of this plan.
4.  ~~Who sits on the `wp-admins` CODEOWNERS/reviewer team for `wp-registry`?~~ **Resolved 2026-08-23 (provisional): xtimbeau, for now.** A single-admin arrangement is acceptable to start; revisit if/when a broader `wp-admins`/`web-admins` team is formalized.
...
13. ~~New, raised 2026-08-22: unregistered/pending WPs should still be publishable to FTP in a "staged" form, not just hard-fail.~~ **Resolved 2026-08-25 (revised from 2026-08-23): FTP staging reinstated.** Server-side confirmation (2026-08-25): `STAGING_USERNAME` is chrooted to `stage/` on the FTP server, so staging credentials can be distributed as org-level secrets without risk of escaping to the production (`wp/`) tree. Versioned FTP staging (see "Staging = FTP" section above) replaces the earlier GitHub Pages-only staging resolution. GitHub Pages remains for the initial pre-request draft state (before `wp_registry_request()` is called) only. Closes the original item; open decision 14 covers the new cleanup/retention question FTP staging introduces.

14. ~~Staging cleanup / retention policy.~~ **Resolved 2026-08-25: keep all versions indefinitely.** All staged versions (`stage/wp/{repo-slug}/v{n}/`) are retained on the FTP server until an explicit admin decision to delete or archive all staging for a given WP. No automatic cleanup, no pruning, no scheduled deletion. Permanent links to any staged version (e.g. sent to reviewers) remain valid until that admin action is taken. The deletion/archival action itself is out of scope for this plan; it is a manual admin operation when the time comes.
