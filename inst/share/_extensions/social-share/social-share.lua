-- d'après social-share de David Schoch
-- modifications des boutons (ajouts de pdf, de bluesky)
-- modification du point d'insertion (::: social-share :::)
-- plus de tolérance aux champs absents
-- modification du css
...
function build_div(m)
  if quarto.doc.is_format("html") then
    ...
    if m.share.pdf then
        -- pdf_url  = string.gsub(share_url, "index.html", pandoc.utils.stringify(m.share.pdf))
        pdf_url  = pandoc.utils.stringify(m.share.pdf)
        share_text = share_text ..
            '  <a href="' ..
            pdf_url ..
            '" download target="_blank" class="pdf"><i class="fa-solid fa-file-pdf fa-fw fa-lg"></i></a><div>     </div>'
    end
