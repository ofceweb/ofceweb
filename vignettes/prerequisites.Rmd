...
enchaîner avec [*Faire un document de travail*](articles/working-papers.html) ou [*Faire un mini-site*](articles/build-a-site.html).
...
`setup_prev()`, `setup_wp()`, `setup_pb()` et `check()` exécutent
...
Ensuite, voir [*faire un mini-site*](articles/build-a-site.html)
pour l'enchaînement `setup_site()` → `render()` → `deploy()`.
