...
#' FTP). Si `FALSE`, lance `quarto publish gh-pages`.
#'
...
site2deploy <- function(
  ...
  # URL finale du site, avec /index.html
  ...
  if(!is.null(site_url) && nzchar(site_url)) {
    ...
    if(!is.null(remotes) && nrow(remotes) > 0) {
      ...
      if(length(m) >= 3)
        final_url <- sprintf("https://%s.github.io/%s/index.html", m[[2]], m[[3]])
    }
  }
