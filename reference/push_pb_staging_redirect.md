...
Équivalent PB de \[push_wp_staging_redirect()\]. Génère un
\`index.html\` de redirection pointant vers la version courante d'un PB
en staging FTP (lue depuis \`website.site-url\` de \`\_quarto.yml\`, de
la forme \`https://staging.ofce.fr/repo/version/\`), le pousse sur la
...
``` r
push_pb_staging_redirect(path = ".", progress = TRUE, trigger = TRUE)
```
...
Ne fait rien si \`website.site-url\` n'a pas la forme attendue (PB
publié, brouillon GitHub Pages, ou brouillon sans version encore
attribuée).

## See also

\[deploy_pb()\], \[push_pb_redirect()\], \[pb_version_up()\]
