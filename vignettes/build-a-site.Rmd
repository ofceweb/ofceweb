...
- `render()` — construit `_site/`,
- `deploy()` — publie le résultat.
...
| `site2branch`  | `FALSE`| Pousse directement vers la branche de déploiement (raccourci équivalent à enchaîner `deploy()`). |
...
`deploy()` (dispatché vers le déploiement `site`) lit la clé `ofce_host` du `_quarto.yml` et choisit :
...
- `push_site_redirect()` — génère et pousse une page `index.html` de
  ...
  segment `/v\d+`). Appelée automatiquement par `publish()` (pour un site
  générique) et `site_version_up()` ; utile à appeler manuellement après une correction
