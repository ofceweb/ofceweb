...
#'   choisie ([render_wp()], [render_site()], [render_prev()] ou
#'   [render_blog()]). Ces fonctions n'ont pas toutes la même signature ;
...
#' @seealso [render_wp()], [render_site()], [render_prev()], [render_blog()],
#'   [detect_repo_type()]
#' @export
...
#' (`"prev"`), d'un blog (`"blog"`) ou d'un site générique (`"site"`).
#' Utilisée par [render()] pour choisir automatiquement la fonction de rendu
#' à appeler.
#'
#' @param root Chemin vers la racine du dépôt (déjà résolu en chemin absolu).
#'
#' @returns Une chaîne : `"wp"`, `"prev"`, `"blog"` ou `"site"`. Si aucun
#'   marqueur n'est trouvé, la fonction s'arrête avec [cli::cli_abort()].
#' @keywords internal
detect_repo_type <- function(root) {
  ...
  if(is_wp && is_prev)
    cli::cli_abort(
      "{.file _quarto.yml} d\u00e9clare \u00e0 la fois {.code ofce_wp: true} et {.code ofce_prev: true} \u2014 configuration incoh\u00e9rente.")
