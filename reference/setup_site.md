...
automatiquement. Juste avant cet appel, \`setup_site()\` vérifie aussi
(via \`check_ofce_version()\`) que le package \*\*ofce\*\* installé est
en version \`\>= 1.3.39\` — sinon la fonction s'arrête immédiatement
avec \[cli::cli_abort()\].
