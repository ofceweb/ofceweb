...
Avant de pouvoir construire et déployer un site avec `ofceweb`, il
faut configurer côté machine quelques éléments d'authentification
GitHub. Ce vignette les passe en revue. Une fois cette configuration
faite, enchaîner avec
[*Construire un site avec ofceweb*](build-a-site.html).
...
- l'outil en ligne de commande **`gh`** (GitHub CLI) authentifié via
  `gh auth login`, utilisé notamment par `encrypt_site()` pour créer
  les secrets de dépôt.

## 1. Qu'est-ce qu'un PAT GitHub ?

Un **Personal Access Token** (PAT) est un jeton qui remplace votre
mot de passe GitHub pour les opérations programmatiques (API, push
HTTPS, workflows). Il est révocable, expirable, et limité à un
ensemble explicite de droits ("scopes").
...
nécessaire pour cette opération. En local, vous pouvez très bien
mettre la même valeur dans `GITHUB_PAT` et `DEPLOY_PAT`.
...
5. **Resource owner** : votre compte, ou l'organisation `OFCE` si
   vous voulez un token valable sur ses dépôts (l'orga doit avoir
   approuvé les fine-grained PAT — sinon prendre un classic PAT).
...
7. **Repository permissions** — minimum requis :
   ...
   - *Secrets* : **Read and write** (uniquement si vous comptez
     utiliser `encrypt_site()` ; sinon laisser à *No access*).
8. *Generate token*.
9. **Copier la valeur affichée** : elle ne sera plus jamais visible.

### Classic PAT (fallback)

Si les fine-grained tokens ne sont pas approuvés par l'organisation,
ou si vous voulez un seul token valable partout :
...
3. Scopes : cocher **`repo`** (couvre push + lecture) et
   **`workflow`** (dispatcher un workflow).
4. *Generate token* → copier la valeur.

## 3. Stocker `GITHUB_PAT` et `DEPLOY_PAT` dans l'environnement

Le but : que ces deux variables soient présentes dans toute session
R que vous démarrez, sans avoir à les retaper.

### Option recommandée : `.Renviron` utilisateur

`.Renviron` est lu par R au démarrage. Le fichier utilisateur vaut
pour toutes vos sessions R, indépendamment du projet.
...
sûr. Un `.Renviron` de projet doit être listé dans `.gitignore` —
c'est ce que fait `encrypt_site()` automatiquement quand il y écrit
`STATICRYPT_PASSWORD`.
...
2. Rouvrir `~/.Renviron` (`usethis::edit_r_environ()`), remplacer la
   valeur, redémarrer R.
3. Si vous utilisiez aussi `gh` avec ce token, refaire `gh auth
   login` (§5).

## 4. Installer `gh` (GitHub CLI)

`gh` est utilisé par `encrypt_site()` pour créer le secret
`STATICRYPT_PASSWORD` sur le dépôt. C'est un binaire indépendant de R.
...
Après installation, **fermer puis rouvrir** le terminal (et RStudio
si lancé) pour que `gh` soit dans le `PATH`. Vérifier :
...
voulez aussi qu'`gh` gère vos clés SSH). Les fine-grained PAT ne
...
À ce stade, `encrypt_site()` peut créer des secrets GitHub via
`gh secret set`, et toute la chaîne `ofceweb` est opérationnelle.
...
pour l'enchaînement `setup_site()` → `render_site()` →
`deploy_site()`.
