...
pas encore le cas, voir [*Pré-requis : PAT GitHub, gh CLI, variables
d’environnement*](https://ofceweb.github.io/ofceweb/articles/prerequisites.md).

## 1. Architecture d’un dépôt de prévision

Un dépôt de prévision porte un nom de la forme `prev{YY}0{3|9}` (ex.
`prev2603` pour mars 2026, `prev2609` pour septembre 2026). Il produit
jusqu’à trois sorties selon le profil Quarto activé :
...
automatiquement `prev`, `annee` et `mois`.
[`setup_prev()`](https://ofceweb.github.io/ofceweb/reference/setup_prev.md)
peut être relancé à tout moment : les fichiers `.qmd` existants et les
...
- Copie les extensions Quarto OFCE et les assets `www/` (toujours mis à
  jour depuis la version du package).
- Crée (si absent) `_quarto.yml`, `_quarto-staging.yml`,
  `_quarto-publish.yml`.
- Copie les gabarits `.qmd` de base dans `france/`, `inter/`, `fiches/`,
  `tableaux_comptes/` (non-destructif).
...
- Définit les variables GitHub `FTP_STAGING_DIR` et `FTP_PUBLISH_DIR`
  via `gh variable set`.

Après
[`setup_prev()`](https://ofceweb.github.io/ofceweb/reference/setup_prev.md),
commiter et pousser avant de rendre le site. Définir le secret
`STATICRYPT_PASSWORD` si ce n’est pas encore fait :
...
personnalisés). Si le secret est absent, le déploiement s’effectue sans
chiffrement.
...
5.  Lance optionnellement un serveur local
    ([`servr::httw()`](https://rdrr.io/pkg/servr/man/httd.html)) pour
...
Un profil personnalisé permet de publier une version alternative du site
(brouillon, revue, variante de scénario…) sur le serveur staging, dans
un sous-dossier dédié, sans toucher aux versions staging et publish
habituelles.
...
nouveau sous-dossier, laissant la version précédente intacte sur le FTP.

Indépendamment de ce numéro de version, le bandeau « Version
préliminaire » affiché sur les pages staging complète automatiquement
son texte avec un compteur de déploiement (« · déploiement n°N »), tiré
de `github.run_number` à chaque exécution de `ftp_deploy_staging.yml` —
utile pour repérer si la dernière mise à jour a bien été poussée, sans
avoir à changer `v0`/`v1`.
...
| [`prev_version_up()`](https://ofceweb.github.io/ofceweb/reference/prev_version_up.md) | Incrémente le segment de version staging |
| [`site2branch()`](https://ofceweb.github.io/ofceweb/reference/site2branch.md) | Primitive git : force-push un dossier vers une branche |
| [`trigger_action()`](https://ofceweb.github.io/ofceweb/reference/trigger_action.md) | Déclenche un workflow GitHub Actions via API |
